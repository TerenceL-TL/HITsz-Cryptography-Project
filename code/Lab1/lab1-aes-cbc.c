...
void mixColumns(int array[4][4])
{
    ...
    for (int c = 0; c < 4; c++) {
        for (int r = 0; r < 4; r++) {
            temp[r][c] = GFMul(colM[r][0], array[0][c]) ^
                          GFMul(colM[r][1], array[1][c]) ^
                          GFMul(colM[r][2], array[2][c]) ^
                          GFMul(colM[r][3], array[3][c]);
        }
    }
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
...
void deMixColumns(int array[4][4])
{
    ...
    for (int c = 0; c < 4; c++) {
        for (int r = 0; r < 4; r++) {
            temp[r][c] = GFMul(deColM[r][0], array[0][c]) ^
                          GFMul(deColM[r][1], array[1][c]) ^
                          GFMul(deColM[r][2], array[2][c]) ^
                          GFMul(deColM[r][3], array[3][c]);
        }
    }
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
...
void addRoundKey(int array[4][4], int round)
{
    ...
    for (int r = 0; r < 4; r++)
    {
        for (int c = 0; c < 4; c++)
        {
            round_key[r][c] = (w[round * 4 + c] >> (24 - r * 8)) & 0x000000ff;
            array[r][c] = array[r][c] ^ round_key[r][c];
        }
    }
}
...
int T(int num, int round)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        sub_temp = (num >> (8 * i)) & 0x000000ff;
        sub_temp = getNumFromSBox(sub_temp);
        res = res ^ sub_temp << (8 * i);
    }
    res = res ^ Rcon[round - 1];
 ...
 * 参数 block2: 密文块或初始向量。
 */
void xorBlock(int block1[4][4], int block2[4][4])
{
    for(int i = 0; i < 4; i++)
    {
        for(int j = 0; j < 4; j++)
...
void aes(char *p, int plen, char *key)
{
    ...
    int pArray[4][4];
    int ivArray[4][4];  // 用来保存 IV 或前一个密文块
    int k,i;
    ...
    // 将 IV 转换为 4x4 的数组
    convertToIntArray(iv, ivArray);

    for(k = 0; k < plen; k += 16)
    {
        convertToIntArray(p + k, pArray);

        xorBlock(pArray, ivArray);
        ...
        memcpy(ivArray, pArray, sizeof(ivArray));
...
void deAes(char *c, int clen, char *key)
{
    ...
    int cArray[4][4];
    int keylen,k;
    int ivArray[4][4], prevCipherBlock[4][4];
    keylen = strlen(key);
    if(clen == 0 || clen % 16 != 0)
    {
    ...
    for(k = 0; k < clen; k += 16)
    {
        ...
        memcpy(prevCipherBlock, cArray, sizeof(prevCipherBlock));

        addRoundKey(cArray, 10);

        deShiftRows(cArray); //行移位 inverse
        ...
        xorBlock(cArray, ivArray);

        memcpy(ivArray, prevCipherBlock, sizeof(ivArray));
